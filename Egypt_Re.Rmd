---
title: "Egypt(including re-exports and re-imports)"
author: "Xiaohan Ma"
date: "2022-12-22"
...
#prepare TradeValue_origin, CIFValue_oecd
 coi.new <-
    ...
    mutate(CIFValue_oecd = case_when(`Trade Flow` == "Export" ~ `Trade Value (US$)` * (1 + value), `Trade Flow` == "Import" ~ `Trade Value (US$)`, `Trade Flow` == "Re-Export" ~ `Trade Value (US$)` * (1 + value), `Trade Flow` == "Re-Import" ~ `Trade Value (US$)`)) %>%
    ...
    mutate(value = tidyr::replace_na(value, x)) %>%
    mutate(CIFValue_unctad = case_when(`Trade Flow` == "Export" ~ `TradeValue_origin` * (1 + value), `Trade Flow` == "Import" ~ `TradeValue_origin`, `Trade Flow` == "Re-Export" ~ `TradeValue_origin` * (1 + value), `Trade Flow` == "Re-Import" ~ `TradeValue_origin`)) %>%
    select(-value) %>%
    left_join(beta_default, by = c("Reporter ISO" = "REPORTER", "Partner ISO" = "PARTNER", "Commodity.code.6" = "HS2012Product", "Year" = "Year")) %>%
    mutate(value = tidyr::replace_na(value, x)) %>%
    mutate(CIFValue_default = case_when(`Trade Flow` == "Export" ~ `TradeValue_origin` * (1 + value), `Trade Flow` == "Import" ~ `TradeValue_origin`,  `Trade Flow` == "Re-Export" ~ `TradeValue_origin` * (1 + value), `Trade Flow` == "Re-Import" ~ `TradeValue_origin`)) %>%
  select(-value)
 ...
 # Mirroring for import data of Egypt
  ...
  if(nrow(coi_re) != 0){
    coi_re <- coi_re %>%
      ...
      mutate(`Trade Flow.y` = ifelse(CIFValue_oecd.y == 0, "Re-Import", `Trade Flow.y`))
  ...
  if(nrow(coi_ri) != 0){
    coi_ri <- coi_ri %>%
      ...
      mutate(`Trade Flow.y` = ifelse(CIFValue_oecd.y == 0, "Re-Export", `Trade Flow.y`))
  ...
  # Combine data
  dspdf <- bind_rows(coi_e, coi_i, coi_ri, coi_re)
...
# calculate absolute and relative difference
  dspdf.new <- dspdf %>%
    ...
    mutate(total_for_all_oecd = tidyr::replace_na(total_for_all_oecd, 0), total_for_all_origin = tidyr::replace_na(total_for_all_origin, 0), total_for_all_unctad = tidyr::replace_na(total_for_all_unctad, 0), total_for_all_default = tidyr::replace_na(total_for_all_default, 0)) %>%
  ...
  write.csv(dspdf.new, file = "dsp_Re.csv")
  save(dspdf.new, file = "dspRe.rda")
    
    
tail(dspdf.new, 100) |> kable()
